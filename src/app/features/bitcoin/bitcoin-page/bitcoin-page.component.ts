...
import { BitcoinService, PricePoint, DcaResult } from '../bitcoin.service';
...
@Component({
  selector: 'app-bitcoin-page',
  standalone: true,
  imports: [DecimalPipe],
  templateUrl: './bitcoin-page.component.html',
  styleUrl: './bitcoin-page.component.scss',
...
export class BitcoinPageComponent {
  ...
  constructor() {
    effect(onCleanup => {
      ...
      const sub = this.bitcoinService.getPriceHistory(days).subscribe({
        next: data => {
      ...
      .subscribe({
        next: prices => {
          ...
          const entries = prices.map(p => {
            const btcBought = amount / p.close;
            cumBtc += btcBought;
            cumInvested += amount;
            return { date: p.date, price: p.close, btcBought, cumBtc, cumInvested, portfolioValue: cumBtc * p.close };
          });
